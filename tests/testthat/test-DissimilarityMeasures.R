...
test_that("Number of outputs CD",{
  expect_equal(nrow(CD(bnfit = travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = "all",covariation = "orderp")$CD),20)
  expect_equal(nrow(CD(bnfit = travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = 0.3,covariation = "all")$CD),1)
  expect_equal(nrow(CD(bnfit = travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = c(0.4,0.5),covariation = "all")$CD),2)
  expect_equal(nrow(CD(bnfit = travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = seq(0.4,0.6,0.05),covariation = "all")$CD),5)
  expect_equal(ncol(CD(bnfit = travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = "all",covariation = "uniform")$CD),2)
  expect_equal(ncol(CD(bnfit = travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = "all",covariation = "proportional")$CD),2)
  expect_equal(ncol(CD(bnfit = travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = "all",covariation = "orderp")$CD),2)
})

test_that("Correct output CD_distance",{
  expect_equal(CD(bnfit = travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = 0.3,covariation = "orderp")$CD[1,2],0.486,tolerance=0.01)
  expect_true(is.na(CD(bnfit = travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = c(0.4,0.6),covariation = "orderp")$CD[2,2]))
})
...
test_that("Correct output KL",{
  expect_equal(KL(travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = 0.3,covariation = "orderp")$KL[1,2],0.012,tolerance=0.01)
  expect_true(is.na(CD(travel,node = "T",value_node = "train",value_parents = c("emp","big"),new_value = c(0.4,0.6),covariation = "orderp")$CD[2,2]))
})
