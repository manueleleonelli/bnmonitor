...
mutual_info <- function(bnfit, node){
  ...
  return(data.frame(Nodes = append(possible, node, after=which(nodes(bnfit)== node)-1), MutalInfo = append(unname(sapply(possible,mi, bnfit= bnfit,node1 = node)),NA,after=which(nodes(bnfit)== node)-1)  ))
}
